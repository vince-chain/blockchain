...
//  - sends over all vince native tokens
...
func (k Keeper) OnRecvPacket(
	...
	// Get addresses in `vince1` and the original bech32 format
	...
	// a sender's own account on vince (sender == recipient)
	...
	if account != nil && vince.IsSupportedKey(account.GetPubKey()) {
		return ack
	}
	...
	k.bankKeeper.IterateAccountBalances(ctx, recipient, func(coin sdk.Coin) (stop bool) {
		...
		err = k.transferKeeper.SendTransfer(
			...
			recipient,                 // recipient is the address in the vince chain
...
// the IBC denomination, i.e port and channel on vince for the voucher. It
// returns an error if:
