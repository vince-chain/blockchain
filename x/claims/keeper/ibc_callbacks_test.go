...
type IBCTestingSuite struct {
	...
	pathEVM    *ibcgotesting.Path // chainA (vince) <-->  chainB (vince)
	pathCosmos *ibcgotesting.Path // chainA (vince) <--> chainCosmos
}
...
func (suite *IBCTestingSuite) SetupTest() {
	...
	suite.chainB.App.(*app.Vince).ClaimsKeeper.SetClaimsRecord(suite.chainB.GetContext(), addr, claimsRecord)

	err = testutil.FundModuleAccount(suite.chainA.App.(*app.Vince).BankKeeper, suite.chainA.GetContext(), types.ModuleName, coins)
	suite.Require().NoError(err)

	suite.chainA.App.(*app.Vince).ClaimsKeeper.SetClaimsRecord(suite.chainA.GetContext(), addr, claimsRecord)

	params := types.DefaultParams()
	params.AirdropStartTime = suite.chainA.GetContext().BlockTime()
	params.EnableClaims = true
	suite.chainA.App.(*app.Vince).ClaimsKeeper.SetParams(suite.chainA.GetContext(), params)
	suite.chainB.App.(*app.Vince).ClaimsKeeper.SetParams(suite.chainB.GetContext(), params)
...
func (suite *IBCTestingSuite) TestOnReceiveClaim() {
	...
	testCases := []struct {
		...
		},
		{
			...
			func(claimableAmount int64) {
				amt := sdk.NewInt(claimableAmount)
				suite.chainB.App.(*app.Vince).ClaimsKeeper.SetClaimsRecord(suite.chainB.GetContext(), senderAddr, types.ClaimsRecord{InitialClaimableAmount: amt, ActionsCompleted: []bool{true, true, true, true}})
			},
			4,
			0,
			true,
		...
		},
		{
			...
			func(claimableAmount int64) {
				amt := sdk.NewInt(claimableAmount)
				suite.chainB.App.(*app.Vince).ClaimsKeeper.SetClaimsRecord(suite.chainB.GetContext(), receiverAddr, types.ClaimsRecord{InitialClaimableAmount: amt, ActionsCompleted: []bool{true, true, true, true}})
			},
			4,
			0,
			true,
...
func (suite *IBCTestingSuite) TestOnAckClaim() {
	sender := "vince1sv9m0g7ycejwr3s369km58h5qe7xj77hvcxrms"
	receiver := "vince1hf0468jjpe6m6vx38s97z2qqe8ldu0njdyf625"
...
func (suite *KeeperTestSuite) TestReceive() {
	...
	secpAddrVince := secpAddr.String()
	secpAddrCosmos := sdk.MustBech32ifyAddressBytes(sdk.Bech32MainPrefix, secpAddr)
	senderStr := "vince1sv9m0g7ycejwr3s369km58h5qe7xj77hvcxrms"
	receiverStr := "vince1hf0468jjpe6m6vx38s97z2qqe8ldu0njdyf625"
...
func (suite *KeeperTestSuite) TestAck() {
	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				addr, err := sdk.AccAddressFromBech32("vince1x2w87cvt5mqjncav4lxy8yfreynn273xn5335v")
				suite.Require().NoError(err)

				mockpacket.Data = transfertypes.ModuleCdc.MustMarshalJSON(
					&transfertypes.FungibleTokenPacketData{
						Sender:   "vince1x2w87cvt5mqjncav4lxy8yfreynn273xn5335v",
